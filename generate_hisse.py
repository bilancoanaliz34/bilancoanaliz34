...
try:
    ...
    for line in reversed(endeks_lines[1:]):
...
window.addEventListener('load', function() {{
  ...
  function initDash(info) {{
    ...
    // Inline logoyu LOGOS objesine geri yükle (var LOGOS={{}} sıfırlamış olabilir)
    document.querySelectorAll('script[data-cfasync]').forEach(function(s){{
      if(s.textContent.indexOf('LOGOS[')>=0){{
...
STATIK_SAYFALAR = [
    ...
    ('/nakit-akis-tablosu.html',          'monthly', '0.8'),
    ('/net-nakit-pozisyonu.html',         'monthly', '0.8'),
